...
router.get('/personalized/newsong', async (req, res) => {
  try {
    ...
    const result = await callNcmModule('personalized/newsong', { limit: parseInt(limit), ...req.query });
    res.json(result);
  } catch (error) {
...
router.get('/intelligence/list', async (req, res) => {
  try {
    ...
    const result = await callNcmModule('playmode/intelligence/list', { id, pid, sid, ...req.query });
    res.json(result);
  } catch (error) {
